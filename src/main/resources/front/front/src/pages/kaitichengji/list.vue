...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0 250px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","justifyContent":"center","overflow":"hidden","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>课题名称：</div>
        <el-input v-model="formSearch.ketimingcheng" placeholder="课题名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('kaitichengji','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#f95927","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/kaitichengjiAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	<div class="list" :style='{"width":"calc(100% - 300px)","padding":"0","margin":"26px 0 20px","background":"#fff","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"23%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"240px"}' class="item">
			<img @click.stop="imgPreView(item.ketifengmian)" v-if="item.ketifengmian && item.ketifengmian.substr(0,4)=='http'" :src="item.ketifengmian" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.ketifengmian?item.ketifengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.ketifengmian?item.ketifengmian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"20px 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"none","textIndent":"2em","overflow":"hidden","flex":"1","borderWidth":"0 1px 0 0","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"240px"}'>{{item.yijianjianyi}}</div>
		      <div class="info">
				<div :style='{"borderColor":"#ddd","overflow":"hidden","whiteSpace":"nowrap","color":"inherit","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">课题名称:{{item.ketimingcheng}}</div>
				<div :style='{"borderColor":"#ddd","overflow":"hidden","whiteSpace":"nowrap","color":"inherit","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">课题分类:{{item.ketifenlei}}</div>
				<div :style='{"borderColor":"#ddd","overflow":"hidden","whiteSpace":"nowrap","color":"inherit","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.jiaoshigonghao}}</div>
		        ...
		        <div :style='{"width":"100%","padding":"0","borderColor":"#ddd","borderStyle":"dashed","borderWidth":"0 0 0px","display":"block"}'>
		          ...
		          <span :style='{"color":"inherit","lineHeight":"24px","fontSize":"inherit"}'>{{item.jiaoshigonghao}}</span>
		        </div>
		      </div>
		    </div>
		  </div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`kaitichengji/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/kaitichengjiDetail', query: params});
      },
