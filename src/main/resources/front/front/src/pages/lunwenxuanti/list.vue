...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0 250px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","justifyContent":"center","overflow":"hidden","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>题目名称：</div>
        <el-input v-model="formSearch.timumingcheng" placeholder="题目名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('lunwenxuanti','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#f95927","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/lunwenxuantiAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	<div class="list" :style='{"width":"calc(100% - 300px)","padding":"0","margin":"26px 0 20px","background":"#fff","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"23%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"240px"}' class="item">
			<img @click.stop="imgPreView(item.tupian)" v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
			...
			<div class="content">
		      <div class="info">
				<div :style='{"borderColor":"#ddd","overflow":"hidden","whiteSpace":"nowrap","color":"inherit","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">题目名称:{{item.timumingcheng}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.timumingcheng != '') searchWhere.timumingcheng = '%' + this.formSearch.timumingcheng + '%';
		if(!this.centerType){
			params['sfsh'] = '是';
		}
        this.$http.get(`lunwenxuanti/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/lunwenxuantiDetail', query: params});
      },
