<template>
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		>
			<template >
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'" label="任务书编号" prop="renwushubianhao">
					<el-input v-model="ruleForm.renwushubianhao" placeholder="任务书编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-else-if="ruleForm.renwushubianhao" label="任务书编号" prop="renwushubianhao">
					<el-input v-model="ruleForm.renwushubianhao" placeholder="任务书编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="date" v-if="type!='info'" label="下任务时间" prop="xiarenwushijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.xiarenwushijian" 
						type="date"
						:readonly="ro.xiarenwushijian"
						placeholder="下任务时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-else-if="ruleForm.xiarenwushijian" label="下任务时间" prop="xiarenwushijian">
					<el-input v-model="ruleForm.xiarenwushijian" placeholder="下任务时间" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-if="type!='info'"  label="任务书内容" prop="renwushuneirong">
					<editor 
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.renwushuneirong" label="任务书内容" prop="renwushuneirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.renwushuneirong"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.renwushuneirong = this.ruleForm.renwushuneirong.replace(reg,'../../../springboot3wd7d5i4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `renwushu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.renwushuCrossAddOrUpdateFlag = false;
						 this.parent.search();
